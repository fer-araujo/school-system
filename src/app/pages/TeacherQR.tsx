      ...
      <div className="w-full max-w-sm bg-white rounded-4xl shadow-2xl border border-slate-100 overflow-hidden flex flex-col">
        ...
        <div className="p-6 sm:p-8 flex flex-col items-center flex-1 bg-slate-50/50">
          <div className="flex justify-center mb-6 w-full">
            <div
              className={`p-4 sm:p-5 rounded-3xl border-4 bg-white transition-all duration-300 shadow-xl ${scanType === "ENTRY" ? "border-emerald-400 shadow-emerald-500/20" : "border-indigo-400 shadow-indigo-500/20"}`}
            >
              {/* 🚨 CORRECCIÓN: Quitamos el style={{...}} y usamos la clase w-full h-auto nativa de tailwind aplicada a un div padre que controla el SVG */}
              <div className="w-full max-w-62.5 aspect-square bg-white flex [&>svg]:w-full [&>svg]:h-auto [&>svg]:max-w-full">
                <QRCode value={qrPayload} size={250} level="H" />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col items-center justify-center space-y-3 mb-8">
            <div
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-normal border transition-colors ${scanType === "ENTRY" ? "text-emerald-700 bg-emerald-100/50 border-emerald-200" : "text-indigo-700 bg-indigo-100/50 border-indigo-200"}`}
            >
              <ShieldCheck size={16} />
          ...
          <div className="flex bg-slate-200/70 p-1.5 rounded-2xl w-full relative mt-auto shadow-inner">
            <button
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-xl transition-all z-10 ${scanType === "ENTRY" ? "text-emerald-700" : "text-slate-500 hover:text-slate-700"}`}
            >
              <LogIn size={18} /> Entrada
            </button>
            <button
              onClick={() => setScanType("EXIT")}
              className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-bold rounded-xl transition-all z-10 ${scanType === "EXIT" ? "text-indigo-700" : "text-slate-500 hover:text-slate-700"}`}
            >
              <LogOut size={18} /> Salida
            </button>
            <div
              className={`absolute top-1.5 bottom-1.5 w-[calc(50%-6px)] bg-white rounded-xl shadow-sm border border-slate-200 transition-transform duration-300 ease-spring ${scanType === "ENTRY" ? "translate-x-0" : "translate-x-full"}`}
            ></div>
          </div>
        </div>
